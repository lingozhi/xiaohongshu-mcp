...
- 启动后，会产生一个 `data/` 目录，用于存储 cookies 和下载的图片/视频。它会挂载到 Docker 容器的 `/data` 目录。
- 如果要使用本地图片/视频发布，请将文件拷贝到 `./data/images/` 目录下，并在 MCP 发布时指定路径为：`/data/images/xxx.jpg`
- 也可以直接使用图片/视频的 URL，服务会自动下载到 `/data/images/` 目录
